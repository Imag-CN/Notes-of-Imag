...
We first consider the case $\mathfrak p\neq \mathfrak m$. Since $\mathfrak p\neq \mathfrak m$, not all of $x,y,z,w$ lie in $\mathfrak p$. W.L.O.G we suppose that $x\notin \mathfrak p$. Then in $R_x$, the element $x$ is invertible, and from $xy=zw$, we get $y=\dfrac{zw}{x}$. Thus $R_{\mathfrak p}$ is a localization of the polynomial ring $\mathbb C[x,z,w]$. The following lemma indicates that $R_{\mathfrak{p}}$ is Cohen-Macaulay:
___
>[!lemma] Lemma
>Let $k$ be a field and $R = k[x_1,\dots,x_n]$ the polynomial ring. For any prime ideal $\mathfrak{p} \subset R$, the localization $A = R_{\mathfrak{p}}$ is a Cohen–Macaulay ring.

**Proof of the lemma:** Let $\mathfrak{m} = \mathfrak{p}R_{\mathfrak{p}}$ be the maximal ideal of $A$ and $d = \dim A = \operatorname{ht}(\mathfrak{p})$ (remark after Corollary 11.12). By Proposition 11.13, there exists a system of parameters $x_1,\dots,x_d \in \mathfrak{m}$ generating an $\mathfrak{m}$-primary ideal. Since $\mathfrak{m}$ is generated by $d$ elements, Theorem 11.22 gives $G_{\mathfrak{m}}(A) \cong k[t_1,\dots,t_d]$. Lemma 11.23 then implies that $x_1,\dots,x_d$ form an $A$-regular sequence, hence $\operatorname{depth} A \ge d$. The reverse inequality $\operatorname{depth} A \le \dim A = d$ holds for any Noetherian local ring (Propositions 11.8 and Corollary 11.9). Thus $\operatorname{depth} A = d = \dim A$, i.e. $A$ is Cohen–Macaulay.
___
