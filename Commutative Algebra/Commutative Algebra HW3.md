...
**Proof:**
Let $R = \text{Im}(A \to B' \otimes_A C)$, and consider the $R$-subalgebra generated by $y = f(b) \otimes c$.

Since $f(b)$ is integral over $A$, the ring $A[f(b)] \subseteq B'$ is a finitely generated $A$-module.

Then $A[f(b)] \otimes_A C$ is a finitely generated $C$-module (hence also an $A$-module), and sits inside $B' \otimes_A C$.

Now, $y = f(b) \otimes c \in A[f(b)] \otimes_A C$, and since the latter is a finitely generated $A$-module, the submodule $A[y]$ is contained in a finitely generated $A$-module, thus $A[y]$ is a finitely generated $A$-module, so $y$ is integral over $A$.

Hence $f \otimes 1$ is integral.
___
