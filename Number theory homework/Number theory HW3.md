...
**Proof:**
**(i)** Denote the map by $\Phi$. Let $G=\operatorname{Gal}(M/K)$, $H=\operatorname{Gal}(M/L)$, and $D=D_{\mathfrak r}(M/K)$. 
...
We compute $|H \backslash G / H|$. Let $G=\left< x,y|x^{3}=y^{2}=1 \right>$ and $H$ generated by $y$. Then the double cosets are $\{ 1,y \}$ and $\{ x,x^{2},xy,x^{2}y \}$. Therefore $|H \backslash G / H|=2$,there are only $2$ primes of $L$ above $\mathfrak{p}$. Hence $\mathfrak{p}$ does not split completely in $L$.
...
H\backslash G/D.  
$$  
Since $\mathfrak p$ is unramified in $L/K$, it is unramified in $M/K$, so $D$ is cyclic and generated by a Frobenius element $\operatorname{Frob}_{\mathfrak r}$.

Under the action of $D$ on $H\backslash G$, the orbit corresponding to a prime $\mathfrak q$ has size $f_{\mathfrak q}$. Thus $f_{\mathfrak q}=1$ means that $\operatorname{Frob}_{\mathfrak r}$ fixes the corresponding element of $H\backslash G$.

By assumption, there are two distinct primes $\mathfrak q,\mathfrak q'$ with  
$$  
f_{\mathfrak q}=f_{\mathfrak q'}=1.  
$$  
Hence $\operatorname{Frob}_{\mathfrak r}$ fixes at least two distinct elements of $H\backslash G$. Since $G$ is a solvable transitive subgroup of $S_p$, the given group-theoretic fact implies  
$$  
