...
>[!lemma] 
>If $h:X\to\mathbb{R}$ is $\mathcal{S}$-measurable and $\varphi:\mathbb{R}\to\mathbb{R}$ is continuous, then $\varphi\circ h$ is $\mathcal{S}$-measurable.  

**Proof:** For any open set $U\subseteq\mathbb{R}$, $\varphi^{-1}(U)$ is open. Hence $(\varphi\circ h)^{-1}(U)=h^{-1}(\varphi^{-1}(U))\in\mathcal{S}$ because $h$ is measurable.

**Back to the Proof of the Problem:**
...
> $$f: X \to [-\infty, \infty]$$
> is a function such that $f^{-1}((a, \infty)) \in \mathcal{S}$ for every $a \in \mathbb{R}$, then $f$ is an $\mathcal{S}$-measurable function.

**Proof:**
Let $\mathcal{A} = \{ E \subseteq [-\infty, \infty] : f^{-1}(E) \in \mathcal{S} \}$.  
One checks that $\mathcal{A}$ is a $\sigma$-algebra on $[-\infty, \infty]$:  

- $f^{-1}([-\infty, \infty]) = X \in \mathcal{S}$, so $[-\infty, \infty] \in \mathcal{A}$.
- If $E \in \mathcal{A}$, then $f^{-1}(E^c) = (f^{-1}(E))^c \in \mathcal{S}$, so $E^c \in \mathcal{A}$.
- If $E_n \in \mathcal{A}$ for $n\in\mathbb{N}$, then $f^{-1}\left(\bigcup_n E_n\right) = \bigcup_n f^{-1}(E_n) \in \mathcal{S}$, so $\bigcup_n E_n \in \mathcal{A}$.

By hypothesis, $(a, \infty) \in \mathcal{A}$ for every $a\in\mathbb{R}$.  

Since the Borel $\sigma$-algebra $\mathcal{B}([-\infty, \infty])$ is generated by the family $\{(a, \infty) : a \in \mathbb{R}\}$, we have $\mathcal{B}([-\infty, \infty]) \subset \mathcal{A}$.  

Thus for any Borel set $B \subset [-\infty, \infty]$, $f^{-1}(B) \in \mathcal{S}$; i.e. $f$ is $\mathcal{S}$-measurable.
___

> [!problem] [SHE] 2B.29
> Give an example of a measurable space $(X, \mathcal{S})$ and a family $\{f_t\}_{t \in \mathbf{R}}$ such that each $f_t$ is an $\mathcal{S}$-measurable function from $X$ to $[0,1]$, but the function $f: X \to [0,1]$ defined by
> $$f(x) = \sup\{f_t(x) : t \in \mathbf{R}\}$$
> is not $\mathcal{S}$-measurable.

**Proof:**
Take $X = \mathbb{R}$, $\mathcal{S} = \mathcal{B}(\mathbb{R})$ (Borel $\sigma$-algebra). Let $A \subset \mathbb{R}$ be a non‑Borel set (e.g., a Vitali set). For each $t \in \mathbb{R}$, define
$$
f_t(x) = \begin{cases}
1 & \text{if } x = t \text{ and } t \in A,\\
0 & \text{otherwise.}
\end{cases}
$$

- Each $f_t$ is $\mathcal{S}$-measurable because $f_t^{-1}(\{1\}) = \{t\}$ if $t \in A$, and $f_t^{-1}(\{1\}) = \varnothing$ if $t \notin A$; in both cases the preimage is a Borel set (a singleton or the empty set), and $f_t^{-1}(\{0\}) = \mathbb{R} \setminus f_t^{-1}(\{1\})$ is also Borel. Hence $f_t$ is Borel measurable.

- The supremum $f(x) = \sup_{t\in\mathbb{R}} f_t(x)$ satisfies
$$
f(x) = \begin{cases}
1 & \text{if } x \in A,\\
0 & \text{if } x \notin A.
\end{cases}
$$
Thus $f = \mathbf{1}_A$ (the indicator function of $A$). Since $A$ is not Borel, $f^{-1}(\{1\}) = A \notin \mathcal{B}(\mathbb{R})$, so $f$ is not $\mathcal{S}$-measurable.

Therefore $(X, \mathcal{S}) = (\mathbb{R}, \mathcal{B}(\mathbb{R}))$ with the family $\{f_t\}_{t\in\mathbb{R}}$ as above gives the required example.