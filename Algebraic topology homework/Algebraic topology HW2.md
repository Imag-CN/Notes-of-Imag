...
> [!problem]
> Lemma 38: Let $g = (g_1, \dots, g_m)$ be a word in $\bigsqcup_{A} G_A$. Suppose
> (1) $\forall i: g_i \neq 1$ and
...
> [!problem]
> Theorem 41 (Abelianization of free groups): The abelianization of $F(x_1, \dots, x_m)$ is isomorphic to $\mathbb{Z} \oplus \cdots \oplus \mathbb{Z}$ ($m$ copies). In other words, there is a homomorphism
> $$F(x) \longrightarrow \mathbb{Z}^m, \quad x_i \mapsto e_i = (0, \dots, 1, \dots, 0)$$
> Compute the kernel. $\square$
**Proof of Lemma 38.**
A word $g = (g_1, \dots, g_m)$ is reduced if it is not equivalent to a shorter word. The equivalence relation is generated by deleting adjacent letters that are in the same factor $G_A$ and whose product in that factor equals the identity. The conditions (1) and (2) in the lemma explicitly forbid the two operations that could shorten the word: (1) forbids removing a letter that is $1$, and (2) forbids combining two adjacent letters from the same factor. Therefore, $g$ is reduced. The proof is an exercise in verifying that no sequence of allowed reductions can shorten a word satisfying these conditions.

**Proof of Theorem 41.**
Let $F = F(x_1, \dots, x_m)$ be the free group on $m$ generators. Its abelianization is $F/[F, F]$, where $[F, F]$ is the commutator subgroup. The universal property of the free group gives a homomorphism $\varphi: F \to \mathbb{Z}^m$ defined by $\varphi(x_i) = e_i$, where $e_i$ is the $i$-th standard basis vector. This map is surjective. Since $\mathbb{Z}^m$ is abelian, $[F, F] \subset \ker(\varphi)$. Conversely, any element in $\ker(\varphi)$ is a word in the $x_i$ whose total exponent sum for each generator is zero. Such a word can be expressed as a product of commutators (a standard fact about free groups), so $\ker(\varphi) \subset [F, F]$. Hence $\ker(\varphi) = [F, F]$, and by the first isomorphism theorem, $F/[F, F] \cong \mathbb{Z}^m$.